...
for data in lst2:
    ...
    for stuff in data:
        alpha=stuff[0]
        beta=stuff[1]
        plt.plot(Y,gama(Y),label='a ='+str(alpha)+',b = '+str(beta))
...
for stuff in lst3:
        alpha=stuff[0]
        beta=stuff[1]
        plt.plot(Y,chi_square(Y),label='k='+str(int(alpha*2)))
plt.legend(prop={'size': 15})
x=x+1
plt.ylim(0,0.5)
plt.xlim(0,8)
...
for k in lst5:
    plt.subplot(5,3,x)
    sample=[]
    sigma=(np.var(mother)**1/2)/k**(1/2)
    for _ in range(500):
        sample.append(int(np.random.choice(mother,k).mean()))
    mu=np.array(sample).mean()
